 ...
 * Author: 佐々木翔
 *
 * Created on 2021/06/29, 17:00
 * Discription　実装試験の姿勢決定系のプログラム
 * 編集履歴
 * 2021/10/18：タイマーの実装
 * 
 */
    ...
    while(1)
    {
        ...
        if(mode == _ChargeMode)
        {
            ...
            if(cnt1 >= 1)
            {
                ...
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA1, SIDL_W, EID8_DATA1, EID0_DATA1);//加速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求


                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA2, SIDL_W, EID8_DATA2, EID0_DATA2);//加速度y書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA3, SIDL_W, EID8_DATA3, EID0_DATA3); //加速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA4, SIDL_W, EID8_DATA4, EID0_DATA4);//加速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA5, SIDL_W, EID8_DATA5, EID0_DATA5);//加速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA6, SIDL_W, EID8_DATA6, EID0_DATA6);//加速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求
        ...
        if(mode == _COMMMode)
        {
            ...
            if(cnt1 >= 1)
            {
                ...
                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA3, SIDL_W, EID8_DATA3, EID0_DATA3); //加速度z書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA4, SIDL_W, EID8_DATA4, EID0_DATA4);//角速度x書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA5, SIDL_W, EID8_DATA5, EID0_DATA5);//角速度y書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求

                Write(_TXB0DLC , 0b00001000);                                       //メッセージサイズ1byte
                Load_TX_ID(_F_TXB0SIDH, SIDH_DATA6, SIDL_W, EID8_DATA6, EID0_DATA6);//角速度z書き込み
                Load_TX_Data(_F_TXB0D0, 8, data);                                   //データ
                RTS0(_CAN_BAUDRATE);                                                //送信要求
